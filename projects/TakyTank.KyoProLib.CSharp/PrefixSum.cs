...
namespace TakyTank.KyoProLib.CSharp
{
	...
	public class SuffixSum
	{
		...
		/// <summary>Build前は元の配列の値が、Build後は累積和の値が格納されている。</summary>
		/// <remarks>
		/// PrefixSumとは違い、N番目がダミーとなるため、インデックスはズレない。
		/// 累積後は以下の様になる。
		/// [0] : [0, N-1]の和
		/// [1] : [1, N-1]の和
		/// [2] : [2, N-1]の和
		...
		public long Sum(int l, int r)
		{
			// 区間[L, N)は [L, R) + [R, N) であるため、[L, R)の和は sum[L, N) - sum[R, N) で計算出来る。 
			// [L, N)は半開区間なので、その和は[L, N-1]の和と等しくなる。
			// 配列のインデックスはズレていないので、配列に渡す値はLそのままになる。Rも同様。
			return _values[l] - _values[r];
		}
		...
		{
			// 全ての区間和を計算するとO(N^2)になるが、
			// 区間の左端を決め打ったときの最適な右端をO(1)で求めることで、全体としてO(N)にする。
			// Sumの実装から分かるとおり、[R, N)までの和を最小にすれば[L, R)が最大になる。
			// つまり、右から順番にLを決め打っていくときに、和が最小の位置を保持しつつ計算していけばよい。
			...
			for (int i = _n - 1; i >= 0; --i) {
				...
				if (value - min > max) {
					max = value - min;
					l = i;
					r = minIndex - 1; // 最小の箇所は引かれる部分なので、求めたい区間として有効なのはその1個左から
				}
